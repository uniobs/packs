...
def salvar_item():
    ...
    destino_imagem = os.path.join('imagenscomprada', imagem_nome)
    shutil.copyfile(imagem_path, destino_imagem)

    # Salva os dados no arquivo CSV
    with open('dadoscomprada.csv', 'a', newline='') as csv_file:
...
def selecionar_imagem():
    imagem_path = filedialog.askopenfilename(filetypes=[('Imagenscomprada', '*.png;*.jpg;*.jpeg')])
    if imagem_path:
